...
class PaymentController extends Controller
{
     ...
     */
    public function approvedZarinpalPrepaid(Request $request)
    {
     ...
     * Process approved subscription plan requests
     */
    public function approvedZarinpalSubscription(Request $request)
    {
        if ($this->sandbox == 'on') {
        ...
        if ($err) {
            ...
            if ($result['Status'] == 100 || $result['Status'] == 101) {
                ...
                toastr()->error(__('There was an error while checking your subscription. Please try again'));
                return redirect(route('user.plan.subscribe', ['id' => $request->plan_id]));

                //echo 'Transation success. RefID:' . $result['RefID'];
            } else {
                toastr()->error(__('There was an error while checking your subscription. Please try again' . $result['Status']));
                return redirect(route('user.plan.subscribe', ['id' => $request->plan_id]));
            }
        }
    }
