...
class ActivationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($status['status'] == true) {
            ...
            Setting::where('name', 'config')->update(['value' => 'Auto']);

            toastr()->success(__('Application license was successfully activated'));
            return redirect()->back();
        } else {
            toastr()->error(__('There was an error while activating your application, please contact support team.' . $status['message']));
            return redirect()->back();
        }
