...
class LicenseController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->api = 'rtl730fede8171340ba9d8db286046576ad';
        $this->pid = '255572';
        $this->domain = url('/');
    }
    ...
    public function activate_license($license, $client, $create_lic = true)
    {
        ...
        if ($res != 1) {
            ...
            $response = ['status' => true, 'message' => $error];
        }
        ...
        if (!empty($create_lic)) {
            if ($response['status']) {
                ...
                $jsonfile = json_encode($act_lic);
                $licfile = trim($jsonfile);
    ...
    public function verify_license($information = false, $license = false, $client = false,$time_based_check = false)
    {
        //return ['status' => true];
  
        if (!empty($license) && !empty($client) && !empty($information) && is_file($this->license_file)) {
            $license_file = json_decode(file_get_contents($this->license_file));
            if ($client === $license_file->username && $license === $license_file->license && $information === 'Auto')
                $result['status'] = true;
            else
                $result['status'] = false;
