				...
				<div class="card-body">
					<div class="d-flex align-items-end justify-content-between">
						<div>
							...
							<h2 class="mb-0"><span class="number-font fs-20">{{ number_format($total_data_monthly['transactions_current_month'][0]['data']) }}</span><span class="ml-2 text-muted fs-11 data-percentage-change"> {{ __('this month') }}</span></h2>
							...
							<table class="table" id="database-backup">
								...
								<tbody>
									@foreach ($result as $data)
										...
										<td>@if(App::currentLocale() === 'fa')
                                            <span class="font-weight-bold">{{ Carbon::datetopersian('d M Y', $data->created_at) }}</span><br><span>{{ date_format($data->created_at, 'H:i A') }}</span>
                                            @else
                                            <span class="font-weight-bold">{{ date_format($data->created_at, 'd M Y') }}</span><br><span>{{ date_format($data->created_at, 'H:i A') }}</span>
                                            @endif</td>
										</tr>
									@endforeach
								</tbody>
